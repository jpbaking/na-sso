...
async def login(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if admin and admin.password_hash and admin.status == "active" and verify_password(password, admin.password_hash):
        from oneauth.sync import credential_handoff
...
document.getElementById('generate-key').onclick=async()=>{{
 ...
 const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([pem],{{type:'application/x-pem-file'}}));a.download='oneauth_ed25519';a.click();URL.revokeObjectURL(a.href);
...
async def enroll_ssh_key(request: Request, public_key: str = Form(""), private_key: str = Form("")):
    ...
    from oneauth.db import get_session
    from oneauth.models import ManagedUser
    from oneauth.security import public_key_from_private
    try:
...
async def server_generate_ssh_key(request: Request):
    ...
    from oneauth.db import get_session
    from oneauth.models import ManagedUser
    from oneauth.security import generate_ssh_keypair
...
def _complete_password_action(account: dict, choice: str, current_password: str, new_password: str, *, decision: bool):
    from oneauth.audit import record_audit
    from oneauth.db import get_session
    from oneauth.models import ManagedUser, PasswordHistory, utcnow
    from oneauth.security import hash_password, validate_password, verify_password
    from oneauth.config import get_settings
