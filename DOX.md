...
- **Managed user lifecycle** — create/edit/assign/disable/delete/restore/purge users and fan changes out to targets. Start: `na_sso/users.py`. Files: `na_sso/lifecycle.py`, `na_sso/operations.py`, `na_sso/sync.py`. Detail in ./na_sso; operator workflow `docs/ADMIN.md`, end-user workflow `docs/USER.md`.
