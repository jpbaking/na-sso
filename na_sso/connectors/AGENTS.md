...
Owns the connector interface, registry, typed results, and OPNsense, Nexus Repository, Nextcloud, Jenkins, GitLab, Gitea, Immich, and SSH implementations. Orchestration and persistence remain in `../sync.py` and `../models.py`.
...
- Membership-capable production connectors accept resolved per-user assignment memberships for ensure, disable, and inspection. Nexus treats them as roles; OPNsense, Nextcloud, and SSH treat them as groups. Jenkins, GitLab, Gitea, and Immich do not claim membership management. Legacy test connectors may use the base fallback and ignore profile intent.
