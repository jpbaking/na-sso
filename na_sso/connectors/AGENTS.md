...
Owns the connector interface, registry, typed results, and OPNsense, Nexus Repository, Nextcloud, Jenkins, GitLab, Gitea, Immich, Nginx Proxy Manager, and SSH implementations. Orchestration and persistence remain in `../sync.py` and `../models.py`.
...
- Membership-capable production connectors accept resolved per-user assignment memberships for ensure, disable, and inspection. Nexus treats them as roles; OPNsense, Nextcloud, and SSH treat them as groups. Jenkins, GitLab, Gitea, Immich, and Nginx Proxy Manager do not claim membership management. Legacy test connectors may use the base fallback and ignore profile intent.
...
- Jenkins support is limited to the built-in local security realm. Because Jenkins core has no realm-independent disable operation, that mutation fails explicitly and never substitutes destructive deletion.
...
- **Jenkins local-realm users** — Creates, discovers, inspects, and explicitly deletes built-in local-realm accounts through administrator actions, while failing unsupported disable safely. Start: `jenkins.py`.
- **GitLab local users** — Manages and inspects GitLab Self-Managed accounts through administrator Users and moderation APIs. Start: `gitlab.py`.
- **Gitea local users** — Manages and inspects accounts through the administrator Users API, including login prohibition for disable. Start: `gitea.py`.
- **Immich local users** — Manages email-identified accounts through stable administrator Users APIs, mapping disable/enable to soft delete/restore. Start: `immich.py`.
- **Nginx Proxy Manager local users** — Mints a short-lived bearer token per operation and manages email-identified accounts through the v2.15.1 Users API, including profile, password, disable, inspection, discovery, and soft deletion. Start: `npm.py`.
